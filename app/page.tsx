...
type Project = { id:string; name:string; brand:string; product:string; market:string; code:string; description:string; partnerTypes:string; signals:string; cooperation:string; metrics:{found:number;qualified:number;ready:number;score:number}; partners:Partner[] };
...
export default function Home() {
  ...
  const switchProject=(id:string)=>{const next=projects.find(x=>x.id===id)??projects[0];setProjectId(id);setSelectedName(next.partners[0].name);setQuery("");setTier("全部层级");notify(`已切换到：${next.name}`)};
  const runDiscovery=()=>{setRunning(true);window.setTimeout(()=>{setRunning(false);notify("发现任务完成，新找到 24 个候选伙伴")},1800)};
  ...
  return <main className="app-shell">
    ...
    <section className="workspace-main">
      <header className="topbar"><div className="project-switch"><span>当前项目</span><select value={projectId} onChange={e=>switchProject(e.target.value)} aria-label="切换项目">{projects.map(x=><option value={x.id} key={x.id}>{x.name}</option>)}</select><i>{project.code}</i></div><div className="top-actions"><button className="icon-btn">?</button><button className="icon-btn">♢<i/></button><button className="primary" onClick={()=>setCampaignOpen(true)}>＋ 新建项目</button></div></header>
    ...
    {campaignOpen&&<div className="modal-backdrop" onMouseDown={()=>setCampaignOpen(false)}><form className="modal" onMouseDown={e=>e.stopPropagation()} onSubmit={e=>{e.preventDefault();setCampaignOpen(false);notify("新项目已创建，AI 正在生成独立画像和搜索策略")}}><button type="button" className="modal-close" onClick={()=>setCampaignOpen(false)}>×</button><span className="ai-badge">新建独立项目</span><h2>创建合作伙伴开发项目</h2><p>每个项目的数据完全独立，包括画像、搜索词、伙伴池、触达记录和效果数据。</p><div className="form-grid"><label>项目名称<input required placeholder="例如：德国宠物护理伙伴开发"/></label><label>品牌名称<input required placeholder="请输入品牌名称"/></label><label>产品名称<input required placeholder="请输入具体产品"/></label><label>目标市场<select><option>美国</option><option>英国</option><option>德国</option><option>全球</option></select></label><label className="wide">伙伴类型<div className="check-row"><span>✓ KOC</span><span>✓ 内容创作者</span><span>✓ 联盟站</span><span>＋ 垂直媒体</span></div></label></div><button className="create-submit">创建项目并生成独立画像 →</button></form></div>}
